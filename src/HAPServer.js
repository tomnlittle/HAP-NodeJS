...
const {EventedHTTPServer} = require('./util/eventedhttp');
...
HAPServer.prototype._onRequest = function(request, response, session, events) {
    ...
    let requestData = bufferShim.alloc(0);
    ...
    // parse request.url (which can contain querystring, etc.) into components, then extract just the path
        ...
        for (const path in HAPServer.handlers)
...
HAPServer.prototype._handlePair = function(request, response, session, events, requestData) {
    ...
    const sequence = objects[HAPServer.Types.SEQUENCE_NUM][0]; // value is single byte with sequence number
...
HAPServer.prototype._handlePairStepOne = function(request, response, session) {
    ...
    // create a new SRP server
        const srpServer = new srp.Server(srpParams, bufferShim.from(salt), bufferShim.from('Pair-Setup'), bufferShim.from(this.accessoryInfo.pincode), key);
...
HAPServer.prototype._handlePairStepTwo = function(request, response, session, objects) {
    ...
    const A = objects[HAPServer.Types.PUBLIC_KEY]; // "A is a public key that exists only for a single login session."
    const M1 = objects[HAPServer.Types.PASSWORD_PROOF]; // "M1 is the proof that you actually know your own password."
...
HAPServer.prototype._handlePairStepThree = function(request, response, session, objects) {
    ...
    const messageData = bufferShim.alloc(encryptedData.length - 16);
    const authTagData = bufferShim.alloc(16);
    ...
    const encSalt = bufferShim.from('Pair-Setup-Encrypt-Salt');
    const encInfo = bufferShim.from('Pair-Setup-Encrypt-Info');

    const outputKey = hkdf.HKDF('sha512', encSalt, S_private, encInfo, 32);

    const plaintextBuffer = bufferShim.alloc(messageData.length);
    ...
    const clientUsername = M5Packet[HAPServer.Types.USERNAME];
    const clientLTPK = M5Packet[HAPServer.Types.PUBLIC_KEY];
    const clientProof = M5Packet[HAPServer.Types.PROOF];
...
HAPServer.prototype._handlePairStepFour = function(request, response, session, clientUsername, clientLTPK, clientProof, hkdfEncKey) {
    ...
    const controllerSalt = bufferShim.from('Pair-Setup-Controller-Sign-Salt');
    const controllerInfo = bufferShim.from('Pair-Setup-Controller-Sign-Info');
...
HAPServer.prototype._handlePairStepFive = function(request, response, session, clientUsername, clientLTPK, hkdfEncKey) {
    ...
    const S_private = session.srpServer.computeK();
    const accessorySalt = bufferShim.from('Pair-Setup-Accessory-Sign-Salt');
    const accessoryInfo = bufferShim.from('Pair-Setup-Accessory-Sign-Info');
    const outputKey = hkdf.HKDF('sha512', accessorySalt, S_private, accessoryInfo, 32);

    const serverLTPK = this.accessoryInfo.signPk;
    const usernameData = bufferShim.from(this.accessoryInfo.username);

    const material = Buffer.concat([outputKey, usernameData, serverLTPK]);
    const privateKey = bufferShim.from(this.accessoryInfo.signSk);
    ...
    const ciphertextBuffer = bufferShim.alloc(message.length);
    const macBuffer = bufferShim.alloc(16);
...
HAPServer.prototype._handlePairVerifyStepOne = function(request, response, session, objects) {
    ...
    const clientPublicKey = objects[HAPServer.Types.PUBLIC_KEY]; // Buffer
    ...
    const encSalt = bufferShim.from('Pair-Verify-Encrypt-Salt');
    const encInfo = bufferShim.from('Pair-Verify-Encrypt-Info');

    const outputKey = hkdf.HKDF('sha512', encSalt, sharedSec, encInfo, 32).slice(0,32);
...
HAPServer.prototype._handlePairVerifyStepTwo = function(request, response, session, events, objects) {
    ...
    const encSalt = bufferShim.from('Control-Salt');
    const infoRead = bufferShim.from('Control-Read-Encryption-Key');
    const infoWrite = bufferShim.from('Control-Write-Encryption-Key');
...
HAPServer.prototype._handleRemotePairVerify = function(request, remoteSession, session) {
    const objects = tlv.decode(request.requestBody);
    const sequence = objects[HAPServer.Types.SEQUENCE_NUM][0]; // value is single byte with sequence number
...
HAPServer.prototype._handlePairings = function(request, response, session, events, requestData) {
    ...
    const objects = tlv.decode(requestData);
    const requestType = objects[HAPServer.Types.REQUEST_TYPE][0]; // value is single byte with request type
    ...
    // but maybe the client wants to change their public key? so we'll emit 'pair' like we just paired again
        ...
        const clientLTPK = objects[HAPServer.Types.PUBLIC_KEY];
...
HAPServer.prototype._handleRemoteAccessories = function(request, remoteSession, session) {
    const deserializedRequest = JSON.parse(request.requestBody);
...
HAPServer.prototype._handleRemoteCharacteristicsWrite = function(request, remoteSession, session, events) {
    const data = JSON.parse(request.requestBody.toString());
