...
// Talk about hacked together -> HomeKitTypes needs to be instantiated but none of the methods called...
const HomeKitTypes = require('./gen/HomeKitTypes');
const {Advertiser} = require('./Advertiser');
const {HAPServer} = require('./HAPServer');
const {AccessoryInfo} = require('./model/AccessoryInfo');
const {IdentifierCache} = require('./model/IdentifierCache');
...
Accessory.prototype.addBridgedAccessory = function(accessory, deferUpdate) {
    ...
    for (const index in this.bridgedAccessories) {
...
Accessory.prototype._sideloadServices = function(targetServices) {
    for (const index in targetServices) {
...
function hapStatus(err) {
    ...
    let value = 0;  // default if not found or
