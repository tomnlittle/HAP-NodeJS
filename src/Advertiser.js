...
Advertiser.prototype.startAdvertising = function(port) {
   ...
   */
    const host = this.accessoryInfo.username.replace(/\:/ig, '_') + '.local';
    const advertiseName = this.accessoryInfo.displayName
    + '-'
...
Advertiser.prototype._computeSetupHash = function() {
    const setupHashMaterial = this.accessoryInfo.setupID + this.accessoryInfo.username;
    const hash = crypto.createHash('sha512');
    hash.update(setupHashMaterial);
    const setupHash = hash.digest().slice(0, 4).toString('base64');
